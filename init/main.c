...
static int set_hardware_id(char *str)
{
	...
	else if ( strcmp("A80_SR4", str) == 0 )
	{
		g_A68_hwID = A80_SR4;
		strcat(hwid_info,str);
		printk("Kernel HW ID = A80_SR4\n");
	}	
	else if ( strcmp("A80_SR5", str) == 0 )
	{
		g_A68_hwID = A80_SR5;
		strcat(hwid_info,str);
		printk("Kernel HW ID = A80_SR5\n");
	}	
	else if ( strcmp("A80_SR6", str) == 0 )
	{
		g_A68_hwID = A80_SR6;
		strcat(hwid_info,str);
		printk("Kernel HW ID = A80_SR6\n");
	}	
	else if ( strcmp("A80_ER", str) == 0 )
	{
		g_A68_hwID = A80_ER;
		strcat(hwid_info,str);
		printk("Kernel HW ID = A80_ER\n");
	}
...
static int set_cpu_id(char *str)
{
	...
	if ( memcmp("7140e1", (str+2) , 6) == 0 )
	{
		g_A68_cpuID = 714;	//APQ8064 V2 WO HDCP
		printk("CPUID = %d\n",g_A68_cpuID);
		strcat(cpurv_info,str);
	}
	else if ( memcmp("7150e1", (str+2) , 6) == 0 )
	{
		g_A68_cpuID = 715;	//APQ8064 V2 W HDCP
		printk("CPUID = %d\n",g_A68_cpuID);
		strcat(cpurv_info,str);
	}	
	else if ( memcmp("7160e1", (str+2) , 6) == 0 )
	{
		g_A68_cpuID = 716;
		printk("CPUID = %d\n",g_A68_cpuID);
		strcat(cpurv_info,str);
