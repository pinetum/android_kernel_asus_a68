...
int IsPhoneSpk = 1;	//ken_cheng@asus.com
...
//ken_cheng@asus.com for A80 speaker +++
static int A80_SPK_control;
static const char *A80_function[] = {"Off", "On"};
static const struct soc_enum A80_enum[] = {
    SOC_ENUM_SINGLE_EXT(2, A80_function),
};
static int A80_get_spk(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    printk("%s: A80_SPK_control = %d", __func__, A80_SPK_control);
    ucontrol->value.integer.value[0] = A80_SPK_control;
    return 0;
}

static int A80_set_spk(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    IsPhoneSpk = 1;
    return 1;
}
...
//ken_cheng@asus.com for P05 +++
static int P05_MIC_control;
static const char *p05_function[] = {"Off", "On"};
static const struct soc_enum p05_enum[] = {
    SOC_ENUM_SINGLE_EXT(2, p05_function),
};
static int P05_get_mic(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    printk("%s: P05_MIC_control = %d", __func__, P05_MIC_control);
    ucontrol->value.integer.value[0] = P05_MIC_control;
    return 0;
}

static int P05_set_mic(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
#ifdef CONFIG_EEPROM_NUVOTON_A80
    int iRetryCount = 5;
    P05_MIC_control = ucontrol->value.integer.value[0];
    printk("%s: Doing P05 mic select %d!!!\n", __func__ ,P05_MIC_control);
    if (P05_MIC_control) {
        msleep(25);
        while (iRetryCount)
        {
            if((AX_MicroP_setGPIOOutputPin(OUT_uP_MIC_SEL, 1)) == 0)
                break;
        ...
        while (iRetryCount)
        {
            if((AX_MicroP_setGPIOOutputPin(OUT_uP_MIC_SEL, 0)) == 0)
                break;
...
static const struct snd_kcontrol_new A80_controls[] = {
    SOC_ENUM_EXT("A68 Speaker", A80_enum[0], A80_get_spk, A80_set_spk),
};

static const struct snd_kcontrol_new P05_controls[] = {
    SOC_ENUM_EXT("P05 MIC", p05_enum[0], P05_get_mic, P05_set_mic),
};
...
//ken_cheng@asus.com +++
    ...
    snd_soc_add_platform_controls(platform, A80_controls, ARRAY_SIZE(A80_controls));
    snd_soc_add_platform_controls(platform, P05_controls, ARRAY_SIZE(P05_controls));
