...
// ASUS_BSP+++ Wenli "Modify for modem restart"
				...
				set_fs(oldfs);
#endif
				subsys_save_reason(sfr_buf, "external_modem");
// ASUS_BSP--- Wenli "Modify for modem restart"
...
static int mdm_configure_ipc(struct mdm_device *mdev)
{
	...
	//ASUS_BSP+++ BennyCheng "config mdm_errfatal & mdm_status pins as wakeup source"
	enable_irq_wake(mdev->mdm_errfatal_irq);
	//ASUS_BSP--- BennyCheng "config mdm_errfatal & mdm_status pins as wakeup source"
...
errfatal_err:
	...
	//ASUS_BSP+++ BennyCheng "config mdm_errfatal & mdm_status pins as wakeup source"
	enable_irq_wake(mdev->mdm_status_irq);
	//ASUS_BSP--- BennyCheng "config mdm_errfatal & mdm_status pins as wakeup source"
