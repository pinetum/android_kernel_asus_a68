...
static int debug_for_test = 0;
module_param(debug_for_test, int, S_IRUGO | S_IWUSR);
#endif
...
void subsys_save_reason(char *reason, char *name)
{
	snprintf(ssr_reason, MAX_SSR_REASON_LEN, "%s:%s", name, reason);
	ASUSEvtlog("[SSR]%s\n", reason);
}
...
{
#ifndef ASUS_SHIP_BUILD
	if (debug_for_test && (strstr(ssr_reason, USER_FATAL) == NULL)) {
		return 1;
	}
#endif
...
static void subsystem_shutdown(struct subsys_device *dev, void *data)
{
	...
	if (dev->desc->shutdown(dev->desc) < 0 && !isSystemShutdown()) { // ASUS_BSP+ Wenli "config subsystem restart"
...
static void subsystem_powerup(struct subsys_device *dev, void *data)
{
	...
	if (dev->desc->powerup(dev->desc) < 0 && !isSystemShutdown()) { // ASUS_BSP+ Wenli "config subsystem restart"
...
// ASUS_BSP+++ Wenli "config subsystem restart"
	pr_info("Restart sequence requested for %s, restart_level = %d.sub_mask = 0x%X\n",
		name, restart_level, sub_mask);
	real_restart_level = is_subsystem_restart_enable(name);
// ASUS_BSP--- Wenli "config subsystem restart"
...
static int __init subsys_restart_init(void)
{
	...
	ssr_reason = kzalloc(MAX_SSR_REASON_LEN, GFP_KERNEL); // ASUS_BSP+ Wenli "config subsystem restart"
