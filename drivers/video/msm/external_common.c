...
static ssize_t hdmi_tx_sysfs_rda_type(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("MDP:HDMI:%s:+++:g_p03State= '%d'\n", __func__, g_p03State);
...
// ASUS_BSP +++ Tingyi "[A80][MyDP] Support Pad resolution"
	...
	if (supported) {
		limit_supported_video_format(&video_format);
		//printk("(#limit_supported_video_format) EDID: format: %d [%s], %s\n",
		//	video_format, video_format_2string(video_format),
		//	supported ? "Supported" : "Not-Supported");			 					
		 }
	 }
#endif
    if (g_A68_hwID < A80_SR3 && (video_format == HDMI_VFRMT_1920x1080p50_16_9 || video_format == HDMI_VFRMT_1920x1080p60_16_9))
	{
	      return;
	}
// ASUS_BSP --- Tingyi "[A80][MyDP] Support Pad resolution"
...
int hdmi_common_read_edid(void)
{
	...
	printk("[HDMI]:hdmi_common_read_edid +++\n");
...
// ASUS_BSP +++ Tingyi "[A80][MyDP] Support Pad resolution"
    ...
    else
    {
        if (g_p03State == ASUS_P03_LOW){
			printk("MDP:HDMI:%s:hdmi_common_read_edid:calling add_supported_video_format(HDMI_VFRMT_1280x800p60_16_10)\n",__func__);
			...
			printk("MDP:HDMI:%s:hdmi_common_read_edid:calling add_supported_video_format(HDMI_VFRMT_1920x1200p60_16_10)\n",__func__);
            add_supported_video_format(&external_common_state->disp_mode_list,HDMI_VFRMT_1920x1200p60_16_10);
        }
        external_common_state->hdmi_sink = TRUE;
    }
// ASUS_BSP --- Tingyi "[A80][MyDP] Support Pad resolution"
...
bool hdmi_common_get_video_format_from_drv_data(struct msm_fb_data_type *mfd)
{
	...
	if (userformat) {
		format = userformat;
		printk("reserved format is %d\n", format);
	} else if (hdmi_prim_resolution) {
		format = hdmi_prim_resolution;
	} else {
		printk("detecting resolution from %dx%d use top 2 bytes of"
...
// ASUS_BSP +++ Tingyi "[A80][MyDP] Support Pad resolution"
          else if (mfd->var_yres == 1200)
			{
                format = HDMI_VFRMT_1920x1200p60_16_10;
			}			
// ASUS_BSP --- Tingyi "[A80][MyDP] Support Pad resolution"
			break;
		}
	...
	if (external_common_state->video_resolution != format)
		printk("[HDMI] switching %s => %s", msm_hdmi_mode_2string(
			external_common_state->video_resolution),
			msm_hdmi_mode_2string(format));
	else
		printk("[HDMI] resolution %s", msm_hdmi_mode_2string(
			external_common_state->video_resolution));
