...
static char lane[2] = {0xBA, 0x02}; /* MIPI 3 lane */
...
static struct dsi_cmd_desc nt35590_cmd_display_on_cmds[] = {
    {DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(init_cmd001), init_cmd001},
    {DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(command_mode), command_mode},
    {DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(lane), lane},
...
int sharp_set_brightness(int value)
{
    ...
    if (g_fb0_dsi_block) {//(mdp4_overlay_dsi_state_get() == ST_DSI_SUSPEND || g_fb0_dsi_block) {
...
//ASUS_BSP: miniporting +++
    ...
    if (bFirst) {
        ...
        cmdreq.cmds = nt35590_cmd_display_on_cmds;
        cmdreq.cmds_cnt = ARRAY_SIZE(nt35590_cmd_display_on_cmds);
...
//ASUS_BSP: miniporting ---
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		cmdreq.cmds = nt35590_video_display_on_cmds;
		cmdreq.cmds_cnt = ARRAY_SIZE(nt35590_video_display_on_cmds);
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
		mipi_dsi_cmdlist_put(&cmdreq);
...
{
    pr_err("asus_mipi_display_off E\n");
