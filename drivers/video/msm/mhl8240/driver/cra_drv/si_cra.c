...
void SiiRegReadBlock ( SiiReg_t virtualAddr, uint8_t *pBuffer, uint16_t count )
{
    ...
    //printk("## SiiRegReadBlock  pPage->busType = (0x%x),  pPage->address = (0x%x), regOffset = 0x%x \n", pPage->busType, (uint8_t)pPage->address, regOffset);
...
//    printk("!! SiiRegRead  pPage->busType = (0x%x),  pPage->address = (0x%x), regOffset = 0x%x\n", pPage->busType, (uint8_t)pPage->address, regOffset);
...
uint8_t SiiRegRead_check ( SiiReg_t virtualAddr , SiiResultCodes_t * ret )
{
    uint8_t             value = 0;
    uint8_t             regOffset = (uint8_t)virtualAddr;
    pageConfig_t        *pPage;
...
//    printk("!! SiiRegRead  pPage->busType = (0x%x),  pPage->address = (0x%x), regOffset = 0x%x\n", pPage->busType, (uint8_t)pPage->address, regOffset);
    ...
    if (ret)
    {
    	*ret = 	craInstance.lastResultCode;
    }
    return( value );
}
...
//    printk("## SiiRegWrite  pPage->busType = (0x%x),  pPage->address = (0x%x), regOffset = 0x%x\n", pPage->busType, (uint8_t)pPage->address, regOffset);
