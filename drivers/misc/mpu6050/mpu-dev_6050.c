...
#define AMI_P05_DEFAULT_CALI_FILE "921368\n  1024   1024   1024\n -2048  -2048  -2048\n   600    600    600\n     0      0      0\n     0      0      0\n     0      0      0\n   100    100    100\n415\n0 0 0 0 0 0"
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] rm A68 PAD e-compass inter offset"
...
static signed char *Pad_Accel_Orient;
...
static signed char Phone_mag_orientation_A80SR1 [9] = { 0, 0, 1, 0, -1, 0, 1, 0, 0 };
static signed char Pad_gyro_orientation_A80SR1 [9] = { 0, -1, 0, -1, 0, 0, 0, 0, -1 };
static signed char Pad_accel_orientation_A80SR1 [9] = { 0, -1, 0, -1, 0, 0, 0, 0, -1 };
static signed char Pad_mag_orientation_A80SR1 [9] = { 0, 1, 0, 0, 0, 1, 1, 0, 0 };
...
DEVICE_ATTR(Gdir_pad, S_IRUGO, read_pad_gdir, NULL);
DEVICE_ATTR(Mdir_pad, S_IRUGO, read_pad_mdir, NULL);
...
static struct attribute *mpu_6050_attr[] = {
	&dev_attr_gyro_6050_status.attr,
        ...
        &dev_attr_Gdir.attr,
        &dev_attr_Mdir.attr,
        &dev_attr_Gdir_pad.attr,
        &dev_attr_Mdir_pad.attr,
	NULL
};
//ASUS_BSP --- Jason Chang "ATD Interface of 9-axis sensor"
...
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] rm A68 PAD e-compass inter offset"
            if((g_A68_hwID >= A80_SR1) && (g_A68_hwID <= A80_PR))
            {
                ...
                printk("[mpu] load A80 pad offset\n");
            }
            else
            {
                strcat(temp_buf, AMI_INTER_OFFSET_PAD);
                printk("[mpu] load A68 pad offset\n");
            }
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] rm A68 PAD e-compass inter offset"
...
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] rm A68 PAD e-compass inter offset"
        if((g_A68_hwID >= A80_SR1) && (g_A68_hwID <= A80_PR))    
             memcpy(temp_buf, AMI_P05_DEFAULT_CALI_FILE, strlen(AMI_P05_DEFAULT_CALI_FILE));
        else
...
// ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] check e-compass struct
                if(asus_g_mldl_cfg_6050->pdata_slave[EXT_SLAVE_TYPE_COMPASS] != NULL)
                    memcpy(asus_g_mldl_cfg_6050->pdata_slave[EXT_SLAVE_TYPE_COMPASS]->orientation, Pad_Mag_Orient, 9);
                else
                    printk(KERN_INFO "[mpu] e-compass not exist!!\n");
// ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] check e-compass struct
                memcpy(asus_g_mldl_cfg_6050->pdata_slave[EXT_SLAVE_TYPE_ACCEL]->orientation, Pad_Accel_Orient, 9);
            }
...
// ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] check e-compass struct
                if(asus_g_mldl_cfg_6050->pdata_slave[EXT_SLAVE_TYPE_COMPASS] != NULL)
                    ...
                    printk(KERN_INFO "[mpu] e-compass not exist!!\n");
// ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] check e-compass struct
                memcpy(asus_g_mldl_cfg_6050->pdata_slave[EXT_SLAVE_TYPE_ACCEL]->orientation, Phone_Accel_Orient, 9);
            }
...
void mpu_get_orientation_6050(void)
{
        switch (g_A68_hwID) 
        {
            ...
            case A80_SR2:
            case A80_SR3:
                Pad_Gyro_Orient = Pad_gyro_orientation_A80SR1;
                Pad_Accel_Orient = Pad_accel_orientation_A80SR1;
                Pad_Mag_Orient = Pad_mag_orientation_A80SR1;
                Phone_Gyro_Orient = Phone_gyro_orientation_A80SR1;
...
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] check phone/pad e-compass status"
	...
	{
		// e-compass ready ?
		if((ami306_phone_compass_status() == 1) || (ami306_pad_compass_status() == 1))
			wake_up_interruptible(&mpu->mpu_event_wait);
		else
			printk(KERN_INFO "[mpu] e-compass not ready\n");
	}
	else
#endif
		wake_up_interruptible(&mpu->mpu_event_wait);
...
// Tingyi
    ...
    if (mldl_cfg->slave[EXT_SLAVE_TYPE_COMPASS] == NULL) {
//ASUS_BSP +++ Jiunhau_Wang "ignore ecompass cmd if ecompass not ready"
        if((cmd == MPU_GET_MPU_PLATFORM_DATA) || (cmd == MPU_GET_EXT_SLAVE_DESCR) ||
           (cmd == MPU_CONFIG_COMPASS) || (cmd == MPU_GET_CONFIG_COMPASS) ||
           (cmd == MPU_READ_COMPASS) || (cmd == ASUS_MPU_READ_ORIENT_STASUS) ||
           (cmd == ASUS_CHECK_COMPASS_CALIBRATION_STATUS)){
            printk(KERN_INFO "[MPU] e-compass not ready !!\n");
            mutex_unlock(&mpu->mutex);
            return -EBUSY;
        }
...
int mpu_probe_6050(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	// Enter_Zhang+++: Don't load mpu sensor driver in CN&CUCC charging mode
...
#endif
#ifdef CONFIG_MICROP_NOTIFIER_CONTROLLER
        notify_register_microp_notifier(&mpu_hs_notifier, "mpu_dev_6050"); //ASUS_BSP Lenter+
#endif
//ASUS_BSP--- Jason Chang "change oritation for pad"
//ASUS_BSP +++ Jason Chang "ATD Interface of 9-axis sensor"
...
#endif
#ifdef CONFIG_MICROP_NOTIFIER_CONTROLLER
        notify_unregister_microp_notifier(&mpu_hs_notifier, "mpu_dev_6050"); //ASUS_BSP Lenter+
#endif
//ASUS_BSP --- Jason Chang "change oritation for pad"
//ASUS_BSP +++ Jason Chang "replace compass calibration by workqueue"
