...
static struct ami306_mod_private_data *phone_private_data = NULL;

//////////////////////////////////////////////////////////////////////////////////////

extern int ami306_file_read_6050(char * file_name, unsigned char *buf, int read_len);
extern int ami306_file_write_6050(char * file_name, unsigned char *buf, int len);
extern int ami306_recover_compass_calibration_file_6050(void);
extern int Check_and_recover_Calibration_file_6050(void);

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"
extern int get_compass_mode(void);
extern int set_compass_mode(int value);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"
			    ...
			    unsigned char reg, unsigned char bits)
{
	...
	result = inv_serial_read_6050(mlsl_handle, pdata->address, reg, 1, &buf);
	if (result) {
	...
	buf |= bits;
	result = inv_serial_single_write_6050(mlsl_handle, pdata->address, reg, buf);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	}
				  ...
				  unsigned long usecs, unsigned long times)
{
	...
	for (; 0 < times; --times) {
		udelay(usecs);
		result = inv_serial_read_6050(mlsl_handle, pdata->address,
					 AMI_REG_STA1, 1, &buf);
			       ...
			       unsigned char *fine)
{
	...
	dat[1] = 0x7f & fine[0];
	dat[2] = 0;
	result = inv_serial_write_6050(mlsl_handle, pdata->address,
				  sizeof(dat), dat);
	dat[0] = AMI_REG_OFFY;
	dat[1] = 0x7f & fine[1];
	dat[2] = 0;
			  ...
			  struct ext_slave_platform_data *pdata)
{
	...
	result = inv_serial_single_write_6050(mlsl_handle, pdata->address,
...
//ASUS_BSP +++ Jason Chang "check and recover compass calibration file when compass resume"
        if(Check_and_recover_Calibration_file_6050())
        {
            ami306_recover_compass_calibration_file_6050();
        }
//ASUS_BSP --- Jason Chang "check and recover compass calibration file when compass resume"
			     ...
			     struct ext_slave_config *data)
{
	...
	default:
		//LOG_RESULT_LOCATION(INV_ERROR_FEATURE_NOT_IMPLEMENTED);
		printk("[ami306]unsupport command\n");
		return INV_ERROR_FEATURE_NOT_IMPLEMENTED;
		break;
	};
...
struct ext_slave_descr *ami306_get_phone_slave_descr(void)
{
    return &ami306_descr;
}

EXPORT_SYMBOL_GPL(ami306_get_phone_slave_descr);
...
{
    struct i2c_client *client = to_i2c_client(dev);
    struct ami306_mod_private_data *compass = i2c_get_clientdata(client);
    ...
    //printk("%s+\n", __func__);
    if(compass->compass_status)
    {
        //printk("%s: have compass\n", __func__);
        do
        {
            inv_serial_single_write_6050(client->adapter, compass->pdata->address,0x1B, 0x92);
            inv_serial_single_write_6050(client->adapter, compass->pdata->address,0x1D, 0x40);
            res = inv_serial_read_6050(client->adapter, compass->pdata->address,0x10, 6, data);
            
            for(i =0; i<6; i++)
                checksum+=data[i];

            retry--;
        }while(checksum==0 && retry >= 0);

        if(res){
                printk("%s: Read compass register fail\n", __FUNCTION__);
                return sprintf(buf, "0x0 0x0 0x0\n");
        }
        else{
                x = (data[1]<<8)|data[0];
                y = (data[3]<<8)|data[2];
    ...
    {
        printk("%s: no compass\n", __func__);
        return sprintf(buf, "0xffff 0xffff 0xffff\n");
    }
}

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"
static ssize_t get_compass_setting_mode(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int value = 0;
	
	value = get_compass_mode();
	return sprintf(buf, "%d\n", value);
}

static ssize_t set_compass_setting_mode(struct device *dev,struct device_attribute *devattr,const char *buf, size_t count)
{	
...
// ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] support SR4
	if(g_A68_hwID >= A80_SR4){
		set_compass_mode(1);
		printk(KERN_INFO "[AMI306] Not supported (HW_ID >= A80_SR4)\n");
		return count;
	}	
// ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] support SR4	
	if (buf[0] == '0')
...
DEVICE_ATTR(compass_6050_mode, S_IRUGO | S_IWUSR, get_compass_setting_mode, set_compass_setting_mode );
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"
static ssize_t read_compass_id(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int value = 0;
	struct i2c_client *client = to_i2c_client(dev);
	
	value = i2c_smbus_read_byte_data(client, 0xF);
	printk("[AMI306] chip ID = (%d) \n", value);
	
	return sprintf(buf, "%d\n", value);
}

DEVICE_ATTR(compass_6050_id, S_IRUGO, read_compass_id, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
...
DEVICE_ATTR(compass_6050_i2c, S_IRUGO, check_compass_i2c, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"

DEVICE_ATTR(compass_6050_status, S_IRUGO, read_compass_status, NULL);
DEVICE_ATTR(compass_6050_raw, S_IRUGO, read_compass_raw, NULL);

static struct attribute *ami306_attr[] = {
        &dev_attr_compass_6050_status.attr,
        &dev_attr_compass_6050_raw.attr,
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"
        &dev_attr_compass_6050_mode.attr,
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] support E-compass phone/pad change commnd"
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"
        &dev_attr_compass_6050_id.attr,
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
        &dev_attr_compass_6050_i2c.attr,
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
        NULL
};
//ASUS_BSP --- Jason Chang "ATD Interface of 9-axis sensor"
			   ...
			   const struct i2c_device_id *devid)
{
    ...
    phone_private_data = private_data;

    //ASUS_BSP +++ Jason "ATD Interface of 9-axis sensor"
    private_data->attrs.attrs = ami306_attr;
    result = sysfs_create_group(&client->dev.kobj, &private_data->attrs);
    if (result) {
            dev_err(&client->dev, "Not able to create the sysfs\n");
            printk("%s:Not able to create the sysfs\n", __FUNCTION__);
    }
    private_data->compass_status = 0;
    //ASUS_BSP --- Jason "ATD Interface of 9-axis sensor"

	result = inv_mpu_register_slave_6050(THIS_MODULE, client, pdata, ami306_get_phone_slave_descr);
	if (result) {
		dev_err(&client->adapter->dev,
...
static int ami306_mod_remove(struct i2c_client *client)
{
	...
	if(private_data->compass_status == 1) {
		inv_mpu_unregister_slave_6050(client, private_data->pdata,ami306_get_phone_slave_descr);
		private_data->compass_status = 0;
	}
...
void ami306_phone_compass_reinit(struct work_struct *work)
{
	int result = 0;
	
	if(phone_private_data->compass_status == 0)
	{
		result = inv_mpu_register_slave_6050(
			THIS_MODULE, phone_private_data->client, phone_private_data->pdata, ami306_get_phone_slave_descr);
		if (result) {
			printk (KERN_INFO "[AMI306][PAD] change info table : pad -> phone fail\n");
			return;
		}else{
			printk(KERN_INFO "[AMI306][PAD] change info table : pad -> phone success\n");
			phone_private_data->compass_status = 1;
		}
	}
	return;
}
EXPORT_SYMBOL_GPL(ami306_phone_compass_reinit);

void ami306_phone_compass_remove(void)
{
	if(phone_private_data->compass_status == 1)
	{
		inv_mpu_unregister_slave_6050(phone_private_data->client, phone_private_data->pdata,ami306_get_phone_slave_descr);
		phone_private_data->compass_status = 0;
	}
	return;
}
EXPORT_SYMBOL_GPL(ami306_phone_compass_remove);

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] check phone/pad e-compass status"
int ami306_phone_compass_status(void)
{
	return phone_private_data->compass_status;
}
EXPORT_SYMBOL_GPL(ami306_phone_compass_status);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] check phone/pad e-compass status"
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("ami306_mod_phone_6050");
