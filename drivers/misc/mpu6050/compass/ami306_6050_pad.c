...
static struct ami306_mod_private_data *pad_private_data = NULL;
...
struct ext_slave_descr *ami306_get_pad_slave_descr(void)
{
    return &ami306_descr;
}

EXPORT_SYMBOL_GPL(ami306_get_pad_slave_descr);
...
static ssize_t read_compass_pad_id(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int value = 0;
	struct i2c_client *client = to_i2c_client(dev);
	
	value = i2c_smbus_read_byte_data(client, 0xF);
	printk("[AMI306][PAD] chip ID = (%d) \n", value);
	
	return sprintf(buf, "%d\n", value);
}

DEVICE_ATTR(compass_6050_pad_id, S_IRUGO, read_compass_pad_id, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
static ssize_t check_compass_pad_i2c(struct device *dev, struct device_attribute *devattr, char *buf)
{
	int value = 0, ret = 0;
	struct i2c_client *client = to_i2c_client(dev);
...
DEVICE_ATTR(compass_6050_pad_i2c, S_IRUGO, check_compass_pad_i2c, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"

DEVICE_ATTR(compass_6050_pad_status, S_IRUGO, read_compass_status, NULL);
DEVICE_ATTR(compass_6050_pad_raw, S_IRUGO, read_compass_raw, NULL);

static struct attribute *ami306_attr[] = {
        &dev_attr_compass_6050_pad_status.attr,
        &dev_attr_compass_6050_pad_raw.attr,
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"
        &dev_attr_compass_6050_pad_id.attr,
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] support SMMI test : get chip ID"
//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
        &dev_attr_compass_6050_pad_i2c.attr,
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][ATD] check i2c status"
        NULL
};
//ASUS_BSP --- Jason Chang "ATD Interface of 9-axis sensor"
...
static int ami306_mod_remove(struct i2c_client *client)
{
	...
	if(private_data->compass_status == 1) {
		inv_mpu_unregister_slave_6050(client, private_data->pdata,ami306_get_pad_slave_descr);
		private_data->compass_status = 0;
	}
...
static const struct i2c_device_id ami306_mod_pad_id[] = {
	{ "p05_ami306", COMPASS_ID_PAD_AMI306 },
	{}
};

MODULE_DEVICE_TABLE(i2c, ami306_mod_pad_id);

void ami306_pad_compass_reinit(struct work_struct *work)
{
	int value;
	int result = 0;
	
	if(pad_private_data->compass_status == 0)
	{
		result = inv_mpu_register_slave_6050(
			THIS_MODULE, pad_private_data->client, pad_private_data->pdata,ami306_get_pad_slave_descr);
		if (result) {
			printk (KERN_INFO "[AMI306][PAD] change info table : phone -> pad fail\n");
			return;
		}
		else
		{	
			printk(KERN_INFO "[AMI306][PAD] change info table : phone -> pad success\n");
			pad_private_data->compass_status = 1;
		}

		// check pad e-compass chip ID
		value = i2c_smbus_read_byte_data(pad_private_data->client, 0xF);
		printk("[AMI306][PAD] chip ID = (%d) \n", value);
	}
	return;
}
EXPORT_SYMBOL_GPL(ami306_pad_compass_reinit);

void ami306_pad_compass_remove(void)
{
	if(pad_private_data->compass_status == 1)
	{
		inv_mpu_unregister_slave_6050(pad_private_data->client, pad_private_data->pdata,ami306_get_pad_slave_descr);
		pad_private_data->compass_status = 0;
	}
	return;
}
EXPORT_SYMBOL_GPL(ami306_pad_compass_remove);

//ASUS_BSP +++ Jiunhau_Wang "[A80][Sensor][NA][Others] check phone/pad e-compass status"
int ami306_pad_compass_status(void)
{
	return pad_private_data->compass_status;
}
EXPORT_SYMBOL_GPL(ami306_pad_compass_status);
//ASUS_BSP --- Jiunhau_Wang "[A80][Sensor][NA][Others] check phone/pad e-compass status"
...
static struct i2c_driver ami306_mod_driver = {
	...
	.id_table = ami306_mod_pad_id,
	.driver = {
		   .owner = THIS_MODULE,
		   .name = "ami306_mod_6050_pad",
		   },
...
static int __init ami306_mod_init(void)
{
	...
	{
		i2c_add_driver(&ami306_mod_driver);
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("ami306_mod_pad_6050");
