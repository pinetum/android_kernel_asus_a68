...
int asus_get_gsensor_status(void)
{
			...
			unsigned char lpa_freq)
{
	...
	b = 1;
	inv_serial_single_write_6050(mlsl_handle, pdata->address,
				MPUREG_ACCEL_MOT_DUR, b);
	/* Setting the cycle bit and LPA wake up freq */
	inv_serial_read_6050(mlsl_handle, pdata->address, MPUREG_PWR_MGMT_1, 1,
			&b);
	b |= BIT_CYCLE | BIT_PD_PTAT;
	inv_serial_single_write_6050(mlsl_handle, pdata->address,
				MPUREG_PWR_MGMT_1,
				b);
	inv_serial_read_6050(mlsl_handle, pdata->address,
...
#if 1
    //printk("%s, ext_sync=0x%x, lpf=0x%x\n", __func__, private_data->mldl_cfg_ref->mpu_gyro_cfg->ext_sync, private_data->mldl_cfg_ref->mpu_gyro_cfg->lpf);
    result = inv_serial_single_write_6050(mlsl_handle, pdata->address,
        MPUREG_CONFIG,
        (unsigned char)MPUREG_CONFIG_VALUE(private_data->mldl_cfg_ref->mpu_gyro_cfg->ext_sync, private_data->mldl_cfg_ref->mpu_gyro_cfg->lpf));
    if (result) {
        LOG_RESULT_LOCATION(result);
        return result;
    }
			...
			unsigned char *data)
{
	...
	result = inv_serial_read_6050(mlsl_handle, pdata->address,
        ...
        //raw[0] = data
        //printk("jason: g raw = %x %x %x %x %x %x\n",data[0],data[1],data[2],data[3],data[4],data[5]);
	return result;
}
