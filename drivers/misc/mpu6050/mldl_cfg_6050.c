...
static int dmp_stop(struct mldl_cfg *mldl_cfg, void *gyro_handle)
{
	...
	result = inv_serial_read_6050(gyro_handle, mldl_cfg->mpu_chip_info->addr,
	...
	result = inv_serial_single_write_6050(gyro_handle,
...
static int dmp_start(struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	result = inv_serial_read_6050(mlsl_handle, mldl_cfg->mpu_chip_info->addr,
				 MPUREG_USER_CTRL, 1, &user_ctrl_reg);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	...
	result = inv_serial_single_write_6050(
		...
		struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	result = inv_serial_read_mem_6050(mlsl_handle, mldl_cfg->mpu_chip_info->addr,
				     memAddr, 1, &prod_rev);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
...
/*{
    ...
    if(0x68 != mpu_address)
    {
        printk("[mpu]: this mpu is not 6050\n");
        IsMPU6050 = false;
    }
    printk("[mpu]: reg 0x%x = 0x%x\n",MPUREG_WHOAMI,mpu_address);
}*/
//ASUS --- Jason
	return result;
}
				 ...
				 int slave_id)
{
	...
	result = inv_serial_read_6050(gyro_handle,
				mldl_cfg->mpu_chip_info->addr,
...
int inv_mpu_set_firmware_6050(struct mldl_cfg *mldl_cfg, void *mlsl_handle,
			 const unsigned char *data, int size)
{
	...
	for (bank = 0; size > 0; bank++,
		...
		result = inv_serial_write_mem_6050(mlsl_handle,
				mldl_cfg->mpu_chip_info->addr,
				((bank << 8) | 0x00),
				write_size,
				data);
		...
		result = inv_serial_read_mem_6050(mlsl_handle,
				mldl_cfg->mpu_chip_info->addr,
				((bank << 8) | 0x00),
				write_size,
				read);
		       ...
		       unsigned long sensors)
{
	...
	/* Write and verify memory */
#if INV_CACHE_DMP != 0
	inv_mpu_set_firmware_6050(mldl_cfg, gyro_handle,
	...
	result = inv_serial_write_6050(gyro_handle, mldl_cfg->mpu_chip_info->addr,
				  7, regs);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
...
int gyro_config_6050(void *mlsl_handle,
...
int gyro_get_config_6050(void *mlsl_handle,
		struct mldl_cfg *mldl_cfg,
		struct ext_slave_config *data)
{
	struct mpu_gyro_cfg *mpu_gyro_cfg = mldl_cfg->mpu_gyro_cfg;
 ...
 *         by this software.
 */
int inv_mpu_open_6050(struct mldl_cfg *mldl_cfg,
		 void *gyro_handle,
		 void *accel_handle,
 ...
 * @return INV_SUCCESS or non-zero error code
 */
int inv_mpu_close_6050(struct mldl_cfg *mldl_cfg,
		  void *gyro_handle,
		  void *accel_handle,
		  void *compass_handle,
 ...
 */
int inv_mpu_resume_6050(struct mldl_cfg *mldl_cfg,
		   void *gyro_handle,
		   void *accel_handle,
		   void *compass_handle,
		   void *pressure_handle,
 ...
 *  @return  INV_SUCCESS or a non-zero error code.
 */
int inv_mpu_suspend_6050(struct mldl_cfg *mldl_cfg,
		    void *gyro_handle,
		    void *accel_handle,
		    void *compass_handle,
		    void *pressure_handle,
...
int inv_mpu_slave_read_6050(struct mldl_cfg *mldl_cfg,
		       void *gyro_handle,
		       void *slave_handle,
		       struct ext_slave_descr *slave,
...
int inv_mpu_slave_config_6050(struct mldl_cfg *mldl_cfg,
			 void *gyro_handle,
			 void *slave_handle,
			 struct ext_slave_config *data,
			 struct ext_slave_descr *slave,
...
int inv_mpu_get_slave_config_6050(struct mldl_cfg *mldl_cfg,
			     void *gyro_handle,
			     void *slave_handle,
			     struct ext_slave_config *data,
			     struct ext_slave_descr *slave,
