...
static void initP01(struct work_struct *work){
	...
	if(isMicroPConnected()==1){
		if(st_CONNECTED!=g_b_isP01Connected){
			...
			if( (g_A68_hwID >= A80_SR4) && (g_A68_hwID <= A80_PR) ){
	...
	if(st_CONNECTED==g_b_isP01Connected){
		...
		//check if pad cap is <=5 %, remove if yes
		if (is_Mode_APROM()) {
			int pad_cap=0;
			uP_nuvoton_read_reg(MICROP_GAUGE_CAP, &pad_cap);		
			printk("uP_Init: cap: %d \r\n", pad_cap);
			if(pad_cap <=5){
					printk("uP_Init: turnoff pad after checking cap\r\n");
...
static int updateMicroPFirmware(unsigned long arg){
        ...
        */
		...
		if(is_Mode_APROM()) {
			status = uP_nuvoton_read_reg(MICROP_OEM_FW_VERSION,&mcirop_ver);
			memcpy(tmp_pro,mcirop_ver+4,1);
			//printk("mcirop_ver=%32s,status = %d,tmp_pro=%s\r\n",mcirop_ver,status,tmp_pro);	
			if(status > 0){
				if(!strcmp(tmp_pro,"A")){//P05E model name --->PFECA
					g_firmwareSize = 32*1024;
				}
				else if(!strcmp(tmp_pro,"E")){//P05C model name --->PFECE
					g_firmwareSize = 16*1024;
				}else{
					printk("The last letter of model name is =%s\r\n",tmp_pro);	
				}
			}
		} else {
				g_firmwareSize = 16*1024;
       ...
       if(g_microp_ver < 0x8305){
                ...
                for (num_updated_bytes = 0; num_updated_bytes < (g_firmwareSize);){
                    ...
                    do{
                            ...
                            if((u32_current_prog_addr>=0 && u32_current_prog_addr <= g_firmwareSize) && 
                                (u32_tmp_addr==u32_current_prog_addr))
                                    break;
                            else
                                    msleep(20);
