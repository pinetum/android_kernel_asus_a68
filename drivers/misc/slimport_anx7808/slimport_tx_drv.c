...
void sp_tx_initialization(void)
{
	...
	DEV_NOTICE("sp_tx_initialization +++\n"); 
...
{
	// for hdmi check bandwidth i2c error
	printk("sp_tx_get_rx_bw +++ %d\n", sp_tx_pd_mode);	
	if (sp_tx_pd_mode)
	{
		*cBw = 0;
		return;
	...
	if (sp_tx_hw_lt_done) {
		...
		if ((bytebuf[0] & 0x07) != 0x07) {
			...
			if (!sp_tx_test_lt) {
				...
				printk("[myDP] after link_training , swing = %d\n", c);
...
//asus bsp: tune hdmi rx eq_boost				
		...
		DEV_NOTICE("--- Downstream ASUS Pad HDMI ---\n");
		}
		else
		{
		...
		DEV_NOTICE("--- Downstream ASUS Pad DP ---\n");
...
#else
bool asus_padstation_exist_realtime(void){return false;}
#endif
 ...
 */
 	...
 	if (!myDP_TV_mode)
 	{
			...
			while(try_count--){
				ret = asus_padstation_exist_realtime();
				printk("[myDP] check Pad status = (%d)\n", ret);
				if(ret==1)
					return ret;
				else
					msleep(200);
...
//ASUS BSP --- : larry lai for pad solution
	...
	if (c & 0x1f) {
		...
		printk("sp_tx_get_dp_connection true\n");
		return TRUE;
	} else {	
		printk("sp_tx_get_dp_connection false\n");
		return FALSE;
		}
}
...
//	if(c1!=0x80||c2!=0x0||c3!=0x0||c4!=0x0||c5!=0x0)
//	printk("sp_tx_int   C1=%x,c2=%x,c3=%x,c4=%x,c5=%x\n",c1,c2,c3,c4,c5);
...
//ANX : (ver0.4)
///	printk("[MyDP hdmi_rx_initialization] HDMI_RX_CHIP_CTRL_REG read value********* %x   \n", c);
...
//ANX : (ver0.4)
///	printk("[MyDP hdmi_rx_initialization] HDMI_RX_CHIP_CTRL_REG write value********* %x   \n", c);
...
//ANX ---: (ver0.4)			
	...
	sp_write_reg(RX_P0, HDMI_RX_INT_STATUS7_REG, c7);
       ...
       if (sp_tx_asus_pad) {
		...
		if(c1!=0x0||c2!=0x0||c3!=0x0||c4!=0x80||c5!=0x0||c6!=0x1||c7!=0x20)
		{
                  printk("[myDP]hdmi rx :c1=%x,c2=%x,c3=%x,c4=%x,c5=%x,c6=%x,c7=%x\n",c1,c2,c3,c4,c5,c6,c7);
...
extern struct anx7808_data *g_anx7808;
