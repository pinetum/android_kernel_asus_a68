		...
		unsigned long status, void *unused)
{
	...
	case 1:
		...
		if (!wake_lock_active(&the_chip->low_voltage_wake_lock)) {
			...
			//wake_lock(&the_chip->low_voltage_wake_lock); //ASUS_BSP +
					...
					int ibat_ua, int vbat_uv)
{
	...
	if (vbat_uv <= (chip->alarm_low_mv + 20) * 1000 &&
		...
		//wake_lock(&chip->low_voltage_wake_lock);//ASUS_BSP Eason_Chang take off low_voltage_wake_lock
	...
	if (vbat_uv > (chip->alarm_low_mv + 20 + BATT_ALARM_ACCURACY) * 1000
		...
		//wake_unlock(&chip->low_voltage_wake_lock);//ASUS_BSP Eason_Chang take off low_voltage_wake_lock
	}
}
...
static int __devinit pm8921_bms_probe(struct platform_device *pdev)
{
	...
	//ASUS_BSP Eason_Chang take off low_voltage_wake_lock+++
	#if 0
