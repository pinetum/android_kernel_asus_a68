...
///---ASUS_BSP Eason_Chang  ASUS BAT Update using Global variable
//ASUS BSP Eason Chang: print temp info+++
#include "../../include/linux/mfd/pm8xxx/pm8xxx-adc.h"
...
//Eason: A68 can't get temp info take off POWER_SUPPLY_PROP_TEMP +++
static enum power_supply_property msm_batt_power_props_Notemp[] = {
...
extern int get_Temp_from_TIgauge(void);//axc_TI_HWgauge_A80.c
#endif
//ASUS BSP Eason_Chang get Temp from TIgauge---
...
void updatePcbThermal(void)
{
	...
	if (rc){
		printk("[BAT][Thermal]:read PcbT error\n");
		gPcbTemp = 0;
	}
	else{
...
void UpdateGlobalValue(void)
{
	...
	if(g_A68_hwID >= A80_SR1){
		updatePcbThermal();//ASUS BSP Eason Chang: print temp info
	}
...
//ASUS BSP Eason_Chang get Cap from TIgauge+++
    ...
    if( (false == g_adc_get_correct_VolCurr)&&(g_A68_hwID < A80_SR1) )
//ASUS BSP Eason_Chang get Cap from TIgauge---
    {
        return -EINVAL;
    }else{    
...
//Eason: A68 can't get temp info take off POWER_SUPPLY_PROP_TEMP +++
	if(A80_EVB <= g_A68_hwID)
	{
		...
		chip->batt_psy.name = "battery";
		chip->batt_psy.type = POWER_SUPPLY_TYPE_BATTERY;
		chip->batt_psy.properties = msm_batt_power_props_Notemp;
		chip->batt_psy.num_properties = ARRAY_SIZE(msm_batt_power_props_Notemp);
		chip->batt_psy.get_property = pm_batt_power_get_property;
		chip->batt_psy.external_power_changed = pm_batt_external_power_changed;
	}
