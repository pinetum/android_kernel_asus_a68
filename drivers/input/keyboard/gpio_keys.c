...
//ASUS BSP austin---
		...
		irq = MSM_GPIO_TO_INT(button->gpio); //ASUS BSP Austin+
...
static ssize_t charger_power_key_proc_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    ...
    if(strncmp(messages, "1", 1) == 0)
    {
		...
		printk("[KeyPad] Enter charger power key manager state!\n");
    }
...
{
    printk("[KeyPad] create_charger_power_key_proc_file\n");
    charger_power_key_proc_file = create_proc_entry(CHARGER_POWER_KEY_PROC_FILE, 0666, NULL);
