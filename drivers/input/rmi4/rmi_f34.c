...
//ASUS_BSP simpson: add for configid +++
...
struct bin_attribute dev_attr_data = {
	.attr = {
		 .name = "data",
//ASUS_BSP simpson: fix for CTS write permission rule +++
					...
					char *buf)
{
	...
	retval = rmi_read_block(fc->rmi_dev, control_base_addr, configid,
			ARRAY_SIZE(configid));

	if (retval < 0) {
		dev_err(&fc->dev, "Could not read configid from 0x%04x.\n",
			control_base_addr);
		return retval;
	}

	return sprintf(buf, "%02x%02x%02x%02x\n",
			configid[0],configid[1],configid[2],configid[3]);
}
//ASUS_BSP simpson: add for configid ---
