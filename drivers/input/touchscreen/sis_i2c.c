...
#endif
	//Desmond ++
	sis_debug(DEBUG_VERBOSE, "[Touch][sis9257] %s++\n", __FUNCTION__);
/*
...
//Desmond--
	...
	else
	{
		...
		{
			dev_err(&client->dev, "[Touch][sis9257] sis_ts_probe: request_irq failed\n");
		}
	}
...
{
	if(g_bIsPadAttach){
		sis_debug(DEBUG_POWER, "[Touch][sis9257] suspend\n");
...
{
	if(g_bIsPadAttach){
		sis_debug(DEBUG_POWER, "[Touch][sis9257] resume\n");
...
*/
		...
		case A80_PR:
			support_sis = 1;
			g_isA80 = 1;
			sis_debug(DEBUG_POWER, "[Touch][sis9257] A80_%d\n", g_A68_hwID);
			break;
		...
		case A80_SR3: //19
			support_sis = 0;
			g_isA80 = 1;
			sis_debug(DEBUG_POWER, "[Touch][sis9257] not support sis9257 driver. A80_%d\n", g_A68_hwID);
			break;

		default:
			support_sis = 1;
	                g_isA80 = 0;
			sis_debug(DEBUG_POWER, "[Touch][sis9257] Unknow HWID\n");
			break;
	}

	//Desmond ++ disable sis9257 driver, if not support
	if(!support_sis)
		return 0;
...
static void attach_padstation_work(struct work_struct *work)
{
	...
	sis_debug(DEBUG_POWER, "[Touch][sis9257] attach_padstation_work()++\n");
