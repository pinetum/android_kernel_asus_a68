...
//ASUS_BSP+++ "for cable run"
static int has_cable_run = 0;
module_param(has_cable_run, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
//ASUS_BSP--- "for cable run"
...
extern void set_antenna_to_pad_main(void);
extern void set_antenna_to_pad_wifi(void);


//ASUS_BSP+++ "for cable run"
int wcnss_Check_Cable_Run(void)
{
    mm_segment_t oldfs;
    struct file *fp=NULL;
    ...
    fp = filp_open("/data/.tmp/has_cable_run", O_RDONLY, 0);
    if( IS_ERR(fp) ) {
        pr_info("[wcnss]: filp_open fail.\n");
    }
    else {
        has_cable_run = 1;
        pr_info("[wcnss]: filp_open OK.\n");
        filp_close(fp, NULL);
    }
...
int wcnss_IsWlanEnabled(void)
{
    pr_info("[wcnss]: wcnss_IsWlanEnabled, (%d).\n", g_wlan_driver_loaded);
	return g_wlan_driver_loaded;
}
EXPORT_SYMBOL(wcnss_IsWlanEnabled);


int wcnss_setWlanEnable(void)
{
    int dev_con_status = 0;

    g_wlan_driver_loaded = 1;
    pr_info("[wcnss]: g_wlan_driver_loaded = 1.\n");

    //ASUS_BSP+++ "for cable run"
    wcnss_Check_Cable_Run();

    if( has_cable_run == 1 ) {
        pr_info("[wcnss]: no switch antenna.\n");
        return g_wlan_driver_loaded;
    }
    //ASUS_BSP--- "for cable run"

    dev_con_status = AX_MicroP_IsP01Connected();
    if( dev_con_status == 1 ) { /* device is connect to P0x */
        set_antenna_to_pad_wifi();
        pr_info("[wlan]: pad is connected, (set_antenna_to_pad_wifi).\n");
    }

    return g_wlan_driver_loaded;
}
EXPORT_SYMBOL(wcnss_setWlanEnable);


int wcnss_setWlanDisable(void)
{
    int dev_con_status = 0;

    g_wlan_driver_loaded = 0;
    pr_info("[wcnss]: g_wlan_driver_loaded = 0.\n");
    ...
    if( dev_con_status == 1 ) { /* device is connect to P0x */
        ...
        pr_info("[wlan]: pad is connected, (set_antenna_to_pad_main).\n");
    }

    return g_wlan_driver_loaded;
}
EXPORT_SYMBOL(wcnss_setWlanDisable);
...
{
	pr_info("[wcnss]: wcnss_reset_intr..\n");
...
{
	pr_info("[wcnss]: Cancel APPS vote for Iris & Riva\n");
...
static void wcnss_nvbin_dnld(void)
{
	...
	if (ret || !nv || !nv->data || !nv->size) {
		pr_err("wcnss: wcnss_nvbin_dnld: request_firmware failed for %s\n", NVBIN_FILE);
		return;
	}
	...
	if (NULL == outbuffer) {
		pr_err("wcnss: wcnss_nvbin_dnld: failed to get buffer\n");
		goto err_free_nv;
	}
	...
	for (count = 0; count < total_fragments; count++) {
		...
		while ((ret == -ENOSPC) && (retry_count <= 3)) {
			...
			//pr_debug("fragment: %d, len: %d, TotFragments: %d, retry_count: %d\n",count, dnld_req_msg->hdr.msg_len,total_fragments, retry_count);
		...
		if (ret < 0) {
			pr_err("wcnss: wcnss_nvbin_dnld: smd tx failed\n");
