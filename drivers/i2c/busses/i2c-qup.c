...
// ASUS_BSP +++ Maggie_Lee "For I2C suspend/resume issue"
...
static bool MICROP_BUS_SUSPENDING = false;
#endif
// ASUS_BSP ---
...
static inline int qup_i2c_request_gpios(struct qup_i2c_dev *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(i2c_rsrcs); ++i) {
		if (dev->i2c_gpios[i] >= 0) {
			...
			if (result) {
				dev_err(dev->dev,
					"[i2c] gpio_request for pin %d failed\
					with error %d\n", dev->i2c_gpios[i],
					result);
				goto error;
			}
...
//ASUS_BSP +++ Maggie Lee GSBI2 and GSBI4 porting
...
//ASUS_BSP +++ Maggie_Lee GSBI2 and GSBI4 porting
...
// ASUS_BSP +++ Maggie_Lee "For Pad I2C suspend/resume issue"
		if( dev->adapter.nr == APQ_8064_GSBI1_QUP_I2C_BUS_ID )	{
...
static int i2c_qup_pm_suspend_runtime(struct device *device)
{
	...
	// ASUS_BSP +++ Maggie_Lee "For Pad I2C suspend/resume issue"
	#ifdef CONFIG_EEPROM_NUVOTON
	if( (dev->adapter.nr == APQ_8064_GSBI1_QUP_I2C_BUS_ID) && !MICROP_BUS_SUSPENDING && AX_MicroP_IsP01Connected() )
	{
...
// ASUS_BSP --- Maggie_Lee
	...
	if (!pm_runtime_enabled(device) || !pm_runtime_suspended(device)) {
		...
		//return ret;	// ASUS_BSP +++ Maggie_Lee "For Pad I2C can not resume issue"
	}
// ASUS_BSP +++ Maggie_Lee "For Pad I2C suspend/resume issue"
#ifdef CONFIG_EEPROM_NUVOTON
	if(dev->adapter.nr == APQ_8064_GSBI1_QUP_I2C_BUS_ID)	{
		printk("pad i2c resuming...\r\n");
		MICROP_BUS_SUSPENDING = false;
